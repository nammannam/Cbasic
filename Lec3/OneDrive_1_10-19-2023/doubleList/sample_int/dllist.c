...
node_t* createNewNode(const data_t data) { //Chỉ tạo node mới chưa có liên kết pre next 
...
root_t llInsertHead(root_t root, const data_t data) {
  ...
  pNewNode->next = root;
  root->prev = pNewNode;
  // pNewNode->prev = NULL;  // all ready set in createNewNode
...
void showNode(const node_t* pNode) {
  if (pNode == NULL) {printf("node is NULL");
  return;}

  showData(pNode->data);
  if (pNode->next == NULL)
    printf("pNode->next is NULL");
  else {
    printf("pNode->next is: "); 
    showData(pNode->next->data);
  }
  if (pNode->prev == NULL) 
    printf("pNode->prev is NULL");
  else {
    printf("pNode->prev is: "); 
    showData(pNode->prev->data);
  }
}